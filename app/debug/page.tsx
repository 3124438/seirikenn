...
export default function AdminPage() {
  ...
  const handleAdd = async () => {
    if (!manualId || !newName) return alert("IDと名前を入力してください");
    ...
    await setDoc(doc(db, "attractions", manualId), {
      name: newName, openTime, closeTime, duration, capacity, slots 
    });

    setManualId(""); setNewName("");
    alert("作成しました！");
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-gray-900 min-h-screen text-white">
      <h1 className="text-2xl font-bold mb-8 text-yellow-400">管理者: クラス設定</h1>
      
      <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 mb-8">
        <h2 className="text-lg font-bold mb-4">出し物を追加</h2>
