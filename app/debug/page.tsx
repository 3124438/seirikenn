        ...
        <div className="mb-6 border-b border-gray-700 pb-4">
          ...
          <details className="bg-gray-800 rounded-lg p-4 border border-gray-700 mb-4" open={isEditing}>
              ...
              <div className="mt-4 pt-4 border-t border-gray-700">
                  <h3 className="text-sm font-bold mb-4 text-gray-300 border-b border-gray-600 pb-2">
                      {isEditing ? `✏️ ${originalId} を編集中` : "新規作成"}
                  </h3>
                  ...
                  <div className="grid gap-4 md:grid-cols-3 mb-4">
                      <div className="flex flex-col">
                          <label className="text-xs text-gray-400 mb-1">会場ID <span className="text-red-500">*</span> (例: 3B)</label>
                          <input 
                              className={`p-2 rounded text-white bg-gray-700 border border-gray-600 focus:border-blue-500 outline-none ${isEditing && manualId !== originalId ? 'ring-2 ring-yellow-500' : ''}`}
                              placeholder="ID" maxLength={3} value={manualId} onChange={e => setManualId(e.target.value)} 
                          />
                      </div>
                      <div className="flex flex-col">
                          <label className="text-xs text-gray-400 mb-1">会場名 <span className="text-red-500">*</span></label>
                          <input 
                              className="bg-gray-700 p-2 rounded text-white border border-gray-600 focus:border-blue-500 outline-none" 
                              placeholder="会場名" value={newName} onChange={e => setNewName(e.target.value)} 
                          />
                      </div>
                      <div className="flex flex-col">
                          <label className="text-xs text-gray-400 mb-1">管理者Pass (5桁) <span className="text-red-500">*</span></label>
                          <input 
                              className="bg-gray-700 p-2 rounded text-white border border-gray-600 focus:border-blue-500 outline-none" 
                              placeholder="Pass" maxLength={5} value={password} onChange={e => setPassword(e.target.value)} 
                          />
                      </div>
                  </div>
                  ...
                  <div className="grid gap-4 md:grid-cols-2 mb-4">
                      <div className="flex flex-col">
                          <label className="text-xs text-gray-400 mb-1">団体名/クラス名 <span className="text-gray-500">(任意)</span></label>
                          <input 
                              className="bg-gray-700 p-2 rounded text-white border border-gray-600 focus:border-blue-500 outline-none" 
                              placeholder="例: 3年B組, サッカー部" value={department} onChange={e => setDepartment(e.target.value)} 
                          />
                      </div>
                      <div className="flex flex-col">
                          <label className="text-xs text-gray-400 mb-1">画像URL (Discord等) <span className="text-gray-500">(任意)</span></label>
                          <input 
                              className="bg-gray-700 p-2 rounded text-white border border-gray-600 focus:border-blue-500 outline-none" 
                              placeholder="https://..." value={imageUrl} onChange={e => setImageUrl(convertGoogleDriveLink(e.target.value))} 
                          />
                      </div>
                  </div>
                  ...
                  {/* 3段目: 説明文 */}
                  <div className="mb-4">
                      <label className="text-xs text-gray-400 mb-1 block">会場説明文 <span className="text-gray-500">(任意: 最大500文字)</span></label>
                      <textarea 
                  ...
                  {isEditing && manualId !== originalId && <div className="text-xs text-yellow-400 font-bold mb-4 bg-yellow-900/30 p-2 rounded border border-yellow-700">⚠️ IDが変更されています。保存すると新しいIDにデータが移動します。</div>}
                  ...
                  <div className="bg-gray-750 p-3 rounded border border-gray-600 mb-4 bg-gray-900/30">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
                          <div className="flex flex-col">
                              <label className="text-[10px] text-gray-400 mb-1">開始時間</label>
                              <input type="time" value={openTime} onChange={e => setOpenTime(e.target.value)} className="bg-gray-700 p-2 rounded text-sm outline-none"/>
                          </div>
                          <div className="flex flex-col">
                              <label className="text-[10px] text-gray-400 mb-1">終了時間</label>
                              <input type="time" value={closeTime} onChange={e => setCloseTime(e.target.value)} className="bg-gray-700 p-2 rounded text-sm outline-none"/>
                          </div>
                          <div className="flex flex-col">
                              <label className="text-[10px] text-gray-400 mb-1">1枠の時間(分)</label>
                              <input type="number" value={duration} onChange={e => setDuration(Number(e.target.value))} className="bg-gray-700 p-2 rounded text-sm outline-none" placeholder="分"/>
                          </div>
                          <div className="flex flex-col">
                              <label className="text-[10px] text-gray-400 mb-1">1枠の組数(定員)</label>
                              <input type="number" value={capacity} onChange={e => setCapacity(Number(e.target.value))} className="bg-gray-700 p-2 rounded text-sm outline-none" placeholder="定員"/>
                          </div>
                      </div>
                      <div className="flex items-center gap-6">
                          <div className="flex items-center gap-2">
                             <label className="text-xs text-gray-400">1組の最大人数:</label>
                             <input type="number" value={groupLimit} onChange={e => setGroupLimit(Number(e.target.value))} className="w-16 bg-gray-700 p-1 rounded text-sm outline-none text-center" />
                          </div>
                          <label className="text-xs text-red-400 flex items-center gap-2 cursor-pointer font-bold bg-red-900/20 px-2 py-1 rounded border border-red-900/50">
                  ...
                  <div className="flex gap-3">
                      <button onClick={handleSave} className="flex-1 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 py-3 rounded font-bold shadow-lg transition transform hover:scale-[1.01]">{isEditing ? "変更内容を保存" : "会場を作成"}</button>
                      {isEditing && <button onClick={resetForm} className="bg-gray-700 px-6 rounded hover:bg-gray-600 transition">キャンセル</button>}
                  </div>
              </div>
          </details>
          ...
          <div className="flex gap-2 items-center bg-gray-800 p-2 rounded border border-gray-600 mb-6">
              ...
              <input className="flex-1 bg-transparent text-white outline-none" placeholder="ユーザーID検索 (例: 7X...)" value={searchUserId} onChange={e => setSearchUserId(e.target.value)} />
          </div>
