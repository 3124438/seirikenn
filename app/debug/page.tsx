...
export default function AdminPage() {
  ...
  // ★変更: Module 1 対応 - システムモード管理
  // 従来の isPaused は互換性のために保持しつつ、systemModeを主とする
  const [systemMode, setSystemMode] = useState("closed"); // "pre_open", "open", "closed"
  
  // 運用モード（false: 時間予約制, true: 順番待ち制）
  const [isQueueMode, setIsQueueMode] = useState(false);

  // ★追加: Module 1 対応 - メニュー管理
  ...
  const resetForm = () => {
    ...
    setDuration(20); setCapacity(3); 
    setSystemMode("closed"); // 初期化
  ...
  const startEdit = (shop: any) => {
    ...
    // システムモードの読み込み (なければ古いisPausedから推測)
    if (shop.systemMode) {
      setSystemMode(shop.systemMode);
    } else {
      setSystemMode(shop.isPaused ? "closed" : "open");
    }

    setIsQueueMode(shop.isQueueMode || false);
    
    // メニューの読み込み
    setMenuList(shop.menu || []);
    
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  // --- メニュー操作ロジック (AdminEditFormへ渡す) ---
  const handleSaveMenu = (newItem: MenuItem) => {
    // IDがない場合は新規作成としてID付与
    const itemWithId = { ...newItem, id: newItem.id || Date.now().toString() };
    
    setMenuList((prev) => {
      // 既存IDがあれば更新、なければ追加
      const exists = prev.find((p) => p.id === itemWithId.id);
      if (exists) {
        return prev.map((p) => (p.id === itemWithId.id ? itemWithId : p));
      }
  ...
  const handleSave = async () => {
    ...
    const data: any = {
      ...
      isPaused: systemMode === "closed", // 互換性のため維持
      menu: menuList, // メニュー配列を保存
